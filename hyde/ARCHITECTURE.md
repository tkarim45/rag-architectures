...
|---|---|---|---|
| `n_hypotheses` | 1 (default): cheapest, deterministic at temp 0 | 3 to 5: averages out per-draft quirks, better recall on ambiguous questions | Only useful with `temperature > 0`, otherwise every draft is identical (Config enforces this). Cost scales linearly. |
| `temperature` | 0.0 (default): reproducible benchmarks | 0.7 to 1.0: diverse drafts for multi-hypothesis runs | Raise together with `n_hypotheses`; a single high-temperature hypothesis just adds variance. |
