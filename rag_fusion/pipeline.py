"""RAG-Fusion pipeline — multi-query reformulation → dense retrieve each → RRF merge → generate.
...
class Pipeline:
    ...
    def retrieve(self, query: str):
        idx = self._index_()
        cids = retrieve(query, idx, self.cfg.top_k, self.cfg.n_queries)
...
def run(query, bundle, k: int = 5):
    ...
    cids = retrieve(query, idx, max(k, 8), Config.n_queries)
