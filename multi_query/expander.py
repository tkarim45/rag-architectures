"""LLM query expansion for multi-query RAG.

Design decisions:

* Structured output goes through ``core.StructuredCaller`` with a validator — the expansion feeds
  *code* (a retrieval fan-out), so it must parse into ``list[str]`` or be repaired, never trusted
  as free-form text.
* The original question is always kept and always first. Expansion exists to add recall; it must
  never be able to lose the user's actual intent, so variants only ever append after it.
* Two dedup layers: exact (casefolded string) inside the validator, then semantic — a variant
  whose embedding cosine against any already-kept query reaches ``dedup_threshold`` would retrieve
  the same neighborhood and only waste a search, so it is dropped. Embeddings from
  ``core.embeddings`` are L2-normalized, so cosine is a plain dot product.
...
class Expansion:
    ...
    dropped: tuple[str, ...] = ()        # variants removed as near-duplicates of a kept query
    fallback: bool = False               # True when LLM output was unusable and we degraded


class QueryExpander:
    """Turns one question into a small, de-duplicated set of search queries."""
    ...
    def expand(self, question: str) -> Expansion:
        with self._tracer.span("multi_query.expand", n_requested=self._n_queries) as span:
            try:
    ...
    def _semantic_dedup(self, question: str,
        ...
        """Greedy filter in LLM order: keep a variant only if its embedding stays below the
        cosine threshold against *every* query already kept (the original included)."""
        kept = [question]
